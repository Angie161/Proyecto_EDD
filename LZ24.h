...
public:
    ...
    void compress(std::string filePath, std::string compressFileName)
    { // Parametros : (Nombre del archivo a comprimir, nombre del archivo comprimido).
        ...
        for (int index = 0; index < data.size(); index++)
        {
            ...
            {
                // Asegúrate de que no se excedan los límites de la cadena
                size_t substring_length = std::min(longestSavedWord - i, data.size() - index);
                word = data.substr(index, substring_length);

                par = trie.search(word);
                parStr = numberToString(index - par.first) + "," + numberToString(par.second) + ";";
                if (par.second != 0 && word.size() > parStr.size())
                { // Mi palabra esta en mi trie y sale acuenta cambiarlo por un par?
                    if (writingPhrase)
                        compressFile << ";" + parStr;
                    else
                        compressFile << parStr;
                ...
                else if (i == longestSavedWord - 1)
                { // No? entonces nomas escribe la palabra mas pequeña que no coincidia.
                    compressFile << word;
                    writingPhrase = true;
                }
            }
            ...
            findWord = true;
            while (findWord)
            { // Mientras encontremos coincidencias.
                contador++;
                findWord = false;
                for (int i = index + 1; i < std::min(index + searchWindowSize, data.size()) && index % cicoSize < searchWindowSize; i++)
                {
                    size_t substring_length = std::min(contador, data.size() - index);
                    if (data.substr(index, substring_length) == data.substr(i, substring_length))
                    {
                        if (contador > longestSavedWord)
                            longestSavedWord = contador; // Si encontre una coincidencia más larga de las que tengo, la cambio.
                        findWord = true;
                    }
                }
            }
            trie.insert(data.substr(index, std::min(longestSavedWord, data.size() - index)), index); // Insertamos la coincidencia en el trie.
            if (index % cicoSize == 0)
            {
    ...
    void decompress(std::string filePath, std::string fileName)
    { // Parametros : (Nombre del archivo a descomprimir, nombre del archivo descomprimido).
        // Extraemos la información del archivo como un string.
        ...
        { // Paso por todos los caracteres de mi archivo comprimido.
            aux = i;
            vA = i;
            vB = -1;
            vC = -1;
            size = -1;
            while (true)
            { // Asigno mis variables para identificar si es una palabra o un par.
            ...
            if (vB == -1)
            { // Si es una palabra, la agrego.
                auxFile += compressedData.substr(vA, size);
            }
            else
            { // Si no lo es, realizo las acciones para copiar un par.
                std::string p = "";
                std::string c = "";
                ...
                ssize_t count = stringToNumber(c);
                ssize_t position = co - stringToNumber(p);
                if (position < 0)
                {
                    position = 0;
                }
                ...
                {
                    if (position + k < auxFile.size())
                    {
                        auxFile += auxFile[position + k];
                    }
                    else
                    {
                        break;
...
private:
    ...
    { // Para convertir un numero en un string, ejemplo: 97 -> "a" , 685 -> "aa".
        std::string str = "";
        while (number != 0)
        {
    ...
    size_t stringToNumber(std::string str)
    { // Para desconvertir un numero en un string, ejemplo: "a" -> 97 , "aa" -> 685.
        size_t number = 0;
